    ...
    <body>
        <main>
            ...
            if ($_POST["submit"] == "Submit") {
                ...
                $portal_config["auth"]["provider"]["core"] = $_POST["auth>provider>core"];
                if (!file_exists($portal_config["auth"]["provider"]["core"])) {
                    $valid = false;
                    echo "<p class='error'>The specified core authentication provider does not appear to exist.</p>";
                }
                $portal_config["auth"]["provider"]["signin"] = $_POST["auth>provider>signin"];
                if (!file_exists($portal_config["auth"]["provider"]["signin"])) {
                    $valid = false;
                    echo "<p class='error'>The specified sign-in page does not appear to exist.</p>";
                }
                $portal_config["auth"]["provider"]["signout"] = $_POST["auth>provider>signout"];
                if (!file_exists($portal_config["auth"]["provider"]["signout"])) {
                    $valid = false;
                    echo "<p class='error'>The specified sign-outpage does not appear to exist.</p>";
                }
                $portal_config["auth"]["provider"]["signup"] = $_POST["auth>provider>signup"];
                if (!file_exists($portal_config["auth"]["provider"]["signup"])) {
                    $valid = false;
                    echo "<p class='error'>The specified sign-up page does not appear to exist.</p>";
                }

                $portal_config["auth"]["access"]["admin"] = array();
                    ...
                    foreach (explode(",", $_POST["auth>access>admin"]) as $user) {
                        if (strlen($user) > 0) {
                            ...
                            if ($user != preg_replace("/[^a-zA-Z0-9]/", '', $user)) { // Verify that this entry only contains permitted characters.
                                ...
                                array_push($portal_config["auth"]["access"]["admin"], $user);
                            }
                        }
                    }
                }
                if (!in_array($username, $portal_config["auth"]["access"]["admin"])) { // Check to see if the current user is no longer in the admin list.
                ...
                $portal_config["auth"]["access"]["whitelist"] = array();
                    ...
                    foreach (explode(",", $_POST["auth>access>whitelist"]) as $user) {
                        if (strlen($user) > 0) {
                            ...
                            if ($user != preg_replace("/[^a-zA-Z0-9]/", '', $user)) { // Verify that this entry only contains permitted characters.
                                ...
                                array_push($portal_config["auth"]["access"]["whitelist"], $user);
                            }
                        }
                    }
                }

                $portal_config["auth"]["access"]["blacklist"] = array();
                    ...
                    foreach (explode(",", $_POST["auth>access>blacklist"]) as $user) {
                        if (strlen($user) > 0) {
                            ...
                            if ($user != preg_replace("/[^a-zA-Z0-9]/", '', $user)) { // Verify that this entry only contains permitted characters.
                                ...
                                array_push($portal_config["auth"]["access"]["blacklist"], $user);
                            }
                        }
                    }
                }
                ...
                if ($_POST["auth>access>mode"] == "whitelist" or $_POST["auth>access>mode"] == "blacklist") { // Check to see if the access mode is set to a valid value.
                    $portal_config["auth"]["access"]["mode"] = $_POST["auth>access>mode"]; // Update the access mode in the configuration.
                } else {
                    $valid = false;
            ...
            <form method="POST">
                ...
                <h4>Provider</h4>
                <label for="auth>provider>core">Core Provider</label>: <input type="text" name="auth>provider>core" id="auth>provider>core" value="<?php echo $portal_config["auth"]["provider"]["core"]; ?>" readonly><br>
                <label for="auth>provider>signin">Sign-In Page</label>: <input type="text" name="auth>provider>signin" id="auth>provider>signin" value="<?php echo $portal_config["auth"]["provider"]["signin"]; ?>"><br>
                <label for="auth>provider>signout">Sign-Out Page</label>: <input type="text" name="auth>provider>signout" id="auth>provider>signout" value="<?php echo $portal_config["auth"]["provider"]["signout"]; ?>"><br>
                <label for="auth>provider>signup">Sign-Up Page</label>: <input type="text" name="auth>provider>signup" id="auth>provider>signup" value="<?php echo $portal_config["auth"]["provider"]["signup"]; ?>"><br>
                <br><h4>Access</h4>
                ...
                <select name="auth>access>mode" id="auth>access>mode">
                    <option value="whitelist" <?php if ($portal_config["auth"]["access"]["mode"] == "whitelist") { echo "selected"; } ?>>Whitelist</option>
